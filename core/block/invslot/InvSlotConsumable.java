...
extends InvSlot {
    ...
    public ItemStack consume(int amount, boolean simulate, boolean consumeContainers) {
        ...
        for (int i = 0; i < this.size(); ++i) {
            ...
            if (stack == null || stack.stackSize < 1 || !this.accepts(stack) || ret != null && !StackUtil.isStackEqualStrict(stack, ret) || stack.stackSize != 1 && !consumeContainers && stack.getItem().hasContainerItem(stack)) continue;
    ...
    public ItemStack damage(int amount, boolean simulate, EntityLivingBase src) {
        ...
        for (int i = 0; i < this.size() && amount > 0; ++i) {
            ...
            if (stack == null || !this.accepts(stack) || !item.isDamageable() || ret != null && (stack.getItem() != ret.getItem() || !ItemStack.areItemStackTagsEqual((ItemStack)stack, ret))) continue;
            if (simulate) {
                stack = stack.copy();
            }
