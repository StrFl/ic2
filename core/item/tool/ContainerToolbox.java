...
extends ContainerHandHeldInventory<HandHeldToolbox> {
    ...
    public ContainerToolbox(EntityPlayer entityPlayer, HandHeldToolbox Toolbox1) {
        super(Toolbox1);
        int col;
        for (col = 0; col < 9; ++col) {
        ...
        for (int row = 0; row < 3; ++row) {
            for (int col2 = 0; col2 < 9; ++col2) {
                this.addSlotToContainer(new Slot((IInventory)entityPlayer.inventory, col2 + row * 9 + 9, 8 + col2 * 18, 84 + row * 18));
            }
        }
        for (col = 0; col < 9; ++col) {
            this.addSlotToContainer(new Slot((IInventory)entityPlayer.inventory, col, 8 + col * 18, 142));
        }
    }
}
